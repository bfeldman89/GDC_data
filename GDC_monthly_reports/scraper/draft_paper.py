...
def race_csv_data_to_airtable():
    ...
    for record in records:
        ...
        pg_no = record['fields']['race_page_no']
        print(f"report: {report}\tpage: {pg_no}")
        csv_file = f'/Users/blakefeldman/code/GDC_data/GDC_monthly_reports/csv/camelot/race_group/camelot_{report}-page-{pg_no}-table-1.csv'
        this_dict = {}
        with open(csv_file, 'r') as this_file:
            reader = csv.DictReader(this_file)
            for row in reader:
...
def current_age_csv_data_to_airtable():
    ...
    for record in records:
        ...
        print(f"report: {report}\tpage: {pg_no}")
        csv_file = f'/Users/blakefeldman/code/GDC_data/GDC_monthly_reports/csv/camelot/current_age/camelot_{report}-page-{pg_no}-table-1.csv'
        this_dict = {}
        with open(csv_file, 'r') as this_file:
            reader = csv.DictReader(this_file)
            for row in reader:
